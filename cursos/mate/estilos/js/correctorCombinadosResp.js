$(document).ready(function() {
	...
	$('.cBotonCorregir').click(function (e){
		...
		$(context).find('.cGetImg.cEsProblema').each(function(index, element) {
            if($.inArray($(this).parents('[id^=cuestion]').attr('id').substr(8,$(this).parents('[id^=cuestion]').attr('id').length),registroCorrectas.correctas)==-1){
				imagenParaColocar="infeliz.png";
				altParaColocar="Resultado incorrecto";
			}else{
				imagenParaColocar="feliz.png";
				altParaColocar="Resultado correcto";
			}
			$(this).append('<img src="../estilos/imagene/'+imagenParaColocar+'" alt="'+altParaColocar+'" class="cImgBienMal">');
		...
		$(context).find('.cGetImg:not(.cEsProblema)').each(function(index,element){
			...
			$(this).parents(".cImgHook").append('<img style="float:right;margin-top:-48px;" src="../estilos/imagene/'+imagenParaColocar+'" alt="'+altParaColocar+'" class="cImgBienMal">');
	...
	function sortabilizar(ambito){
		var cOrdenable = ambito.find('.cOrdenable');
		if (cOrdenable != undefined ) {
			cOrdenable.sortable({
				cancel: "li[id^=sep]",
